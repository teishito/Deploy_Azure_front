...
export default function SearchForm() {
  ...
  const handleSearch = async (e) => {
    ...
    const query = {
      ...
      budgetMin: budgetMin || "",
      budgetMax: budgetMax || "",
      privateRoom: privateRoom ? "yes" : "",
      drinkIncluded: drinkIncluded ? "yes" : ""
    };

    const queryString = new URLSearchParams(query).toString();

    try {
      const response = await fetch(`https://tech0-gen-8-step3-app-node-10.azurewebsites.net/api/search?${queryString}`, {
        method: 'GET',
      });
